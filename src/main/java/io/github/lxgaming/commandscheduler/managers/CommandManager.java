...
package io.github.lxgaming.commandscheduler.managers;

import io.github.lxgaming.commandscheduler.CommandScheduler;
import io.github.lxgaming.commandscheduler.commands.AbstractCommand;
import io.github.lxgaming.commandscheduler.util.Toolbox;
...
public final class CommandManager {
    ...
    public static CommandResult process(AbstractCommand abstractCommand, CommandSource commandSource, String message) {
        ...
        CommandScheduler.getInstance().getLogger().debug("Processing {} for {}", command.get().getPrimaryAlias().orElse("Unknown"), commandSource.getName());
    ...
    public static boolean registerCommand(Class<? extends AbstractCommand> commandClass) {
        if (getCommandClasses().contains(commandClass)) {
            CommandScheduler.getInstance().getLogger().warn("{} has already been registered", commandClass.getSimpleName());
            return false;
        }
        ...
        if (!command.isPresent()) {
            CommandScheduler.getInstance().getLogger().error("{} failed to initialize", commandClass.getSimpleName());
            return false;
        }
        
        getCommands().add(command.get());
        Sponge.getCommandManager().register(CommandScheduler.getInstance().getPluginContainer(), command.get(), command.get().getAliases().toArray(new String[0]));
        CommandScheduler.getInstance().getLogger().debug("{} registered", commandClass.getSimpleName());
        return true;
    }
    ...
    public static boolean registerAlias(AbstractCommand command, String alias) {
        if (Toolbox.containsIgnoreCase(command.getAliases(), alias)) {
            CommandScheduler.getInstance().getLogger().warn("{} has already been registered for {}", alias, command.getClass().getSimpleName());
            return false;
        }
        
        command.getAliases().add(alias);
        CommandScheduler.getInstance().getLogger().debug("{} registered for {}", alias, command.getClass().getSimpleName());
        return true;
    }
    ...
    public static boolean registerCommand(AbstractCommand parentCommand, Class<? extends AbstractCommand> commandClass) {
        if (parentCommand.getClass() == commandClass) {
            CommandScheduler.getInstance().getLogger().warn("{} attempted to register itself", parentCommand.getClass().getSimpleName());
            return false;
        }
        ...
        CommandScheduler.getInstance().getLogger().debug("{} registered for {}", commandClass.getSimpleName(), parentCommand.getClass().getSimpleName());
        return true;
    }
